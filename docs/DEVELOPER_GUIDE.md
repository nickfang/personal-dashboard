...
We use **gRPC** for internal service-to-service communication. To ensure decoupling and clean dependency management, we follow the **Centralized Gen** pattern.

### 1. Structure
*   **`/services/protos` (The Source of Truth):** Contains raw `.proto` files. Organized by service name and version.
    *   Example: `services/protos/weather-provider/v1/weather_provider.proto`
*   **`/services/gen/go` (The Generated Code):** Contains the Go code generated by `protoc`. This is treated as a shared library module.
    *   Example: `services/gen/go/weather-provider/v1/weather_provider.pb.go`

### 2. Generating Code
We use a "smart" Makefile at the root to handle code generation. It only regenerates files that have changed.
...
### 3. Usage in Services
Services should import the generated code from the `gen/go` module.

```go
import "github.com/nickfang/personal-dashboard/services/gen/go/weather-provider/v1"
```
