...
func collectAll(ctx context.Context, apiKey string, collector *service.CollectorService, locations []shared.Location) error {
	...
	for _, loc := range locations {
		if err := collector.Collect(ctx, apiKey, loc); err != nil {
			slog.Error("Failed to collect pollen", "location", loc.ID, "error", err)
			continue
		}
		successCount++
		slog.Info("Processed pollen", "location", loc.ID)
	}

	if successCount == 0 {
		return fmt.Errorf("all %d locations failed", len(locations))
	}
	slog.Info("Pollen collection complete", "succeeded", successCount, "total", len(locations))
	return nil
}
